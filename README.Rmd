...
s = db |>
  dplyr::slice(1:4) |>
  read_copernicus(path)
s
```

The database directory structure looks like this (requires [fs package](https://CRAN.R-project.org/package=fs)) which you won't need just to use copernicus).
